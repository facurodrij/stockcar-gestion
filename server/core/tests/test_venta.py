...
def test_create_venta(client, new_cliente, new_tipo_comprobante, new_punto_venta):
    data = {
        "numero": 1,
    ...
    venta_data = schema.load(data, session=db.session)
    db.session.add(venta_data)
    db.session.commit()
...
def test_update_venta(client, new_cliente, new_tipo_comprobante, new_punto_venta):
    new_data = {
        "numero": 1,
        "nombre_cliente": "Test Cliente",
        "cliente": new_cliente.id,
    ...
    schema = VentaFormSchema()
    venta_data = schema.load(new_data, session=db.session)
    db.session.add(venta_data)
    db.session.commit()
    print("Total de Venta creada: ", venta_data.total)

    venta = db.session.query(Venta).first()
    update_data = {
        "numero": 1,
        "nombre_cliente": "Test Cliente",
        "cliente": new_cliente.id,
    ...
    update_venta_data = update_schema.load(
        update_data, instance=venta, session=db.session
    )
    db.session.add(update_venta_data)
    db.session.commit()
